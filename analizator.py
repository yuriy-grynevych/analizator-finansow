...
# --- NOWE FUNKCJE POMOCNICZE DO WYNAGRODZEŃ (EXCEL) ---
MAPA_MIESIECY_PL = {
    'STYCZEŃ': 1, 'STYCZEN': 1, 'JANUARY': 1, '01': 1, '1': 1,
    'LUTY': 2, 'FEBRUARY': 2, '02': 2, '2': 2,
    'MARZEC': 3, 'MARCH': 3, '03': 3, '3': 3,
    'KWIECIEŃ': 4, 'KWIECIEN': 4, 'APRIL': 4, '04': 4, '4': 4,
    'MAJ': 5, 'MAY': 5, '05': 5, '5': 5,
    'CZERWIEC': 6, 'JUNE': 6, '06': 6, '6': 6,
    'LIPIEC': 7, 'JULY': 7, '07': 7, '7': 7,
    'SIERPIEŃ': 8, 'SIERPIEN': 8, 'AUGUST': 8, '08': 8, '8': 8,
    'WRZESIEŃ': 9, 'WRZESIEN': 9, 'SEPTEMBER': 9, '09': 9, '9': 9,
    'PAŹDZIERNIK': 10, 'PAZDZIERNIK': 10, 'OCTOBER': 10, '10': 10,
    'LISTOPAD': 11, 'NOVEMBER': 11, '11': 11,
    'GRUDZIEŃ': 12, 'GRUDZIEN': 12, 'DECEMBER': 12, '12': 12
}
...
def wyznacz_zakres_dat_z_arkusza(nazwa_arkusza, nazwa_pliku, rok_domyslny):
    """
    Szuka daty (miesiąca) zarówno w nazwie arkusza, jak i nazwie pliku.
    Priorytet: Nazwa arkusza -> Nazwa pliku.
    """
    # Łączymy nazwy, żeby przeszukać oba miejsca, ale arkusz jest ważniejszy
    tekst_do_analizy = (str(nazwa_arkusza) + " " + str(nazwa_pliku)).upper().strip()
    
    znaleziony_rok = rok_domyslny
    
    # 1. Szukanie roku (2023, 2024, 2025...)
    match_rok = re.search(r'(202[0-9])', tekst_do_analizy)
    if match_rok:
        znaleziony_rok = int(match_rok.group(1))
        # Usuwamy rok, żeby nie mylił np. 01.2025 z dniem 01
        tekst_do_analizy = tekst_do_analizy.replace(match_rok.group(1), "") 

    # 2. Szukanie miesiąca (Słownie lub cyfrowo)
    miesiac = MAPA_MIESIECY_PL.get(str(nazwa_arkusza).upper().strip()) # Najpierw ścisłe dopasowanie arkusza
    
    if not miesiac:
        # Szukanie w połączonym tekście słów kluczowych
        for k, v in MAPA_MIESIECY_PL.items():
            if k in tekst_do_analizy:
                miesiac = v
                break
    
    if not miesiac:
        # Szukanie cyfr np. .01. lub -01-
        match_miesiac = re.search(r'[^0-9](0?[1-9]|1[0-2])[^0-9]', " " + tekst_do_analizy + " ")
        if match_miesiac:
...
def parsuj_dataframe_plac(df):
    """
    Parsuje Excel z płacami, obsługując scalone komórki (screenshot Grudzień).
    """
    # 1. Konwersja na string i czyszczenie
    df = df.astype(str)
    
    # 2. KLUCZOWA ZMIANA: Uzupełnianie scalonych komórek w dół (ffill)
    # Zamieniamy stringi 'nan' na prawdziwy NaN, robimy ffill i wracamy
    df = df.replace(['nan', 'NaN', 'NAN'], np.nan)
    
    # Uzupełniamy pierwsze 3 kolumny w dół (tam zwykle są nazwiska w scalonych komórkach)
    # Dzięki temu przy wierszu "KWOTA" nazwisko będzie widoczne
    df.iloc[:, 0:3] = df.iloc[:, 0:3].ffill()
    
    df = df.fillna('') # Resztę pustych na pusty string dla łatwiejszej obróbki
    
    wyniki = []
    
    # Słowa kluczowe oznaczające wiersz z pieniędzmi
    SŁOWA_KLUCZOWE = ["KWOTA", "DO WYPŁATY", "SUMA", "RAZEM", "DO WYPLATY"]
    
    for idx, row in df.iterrows():
        # Sklejamy tekst wiersza, żeby znaleźć słowo kluczowe
        tekst_wiersza = " ".join(row.iloc[0:5].astype(str)).upper()
        
        if any(s in tekst_wiersza for s in SŁOWA_KLUCZOWE):
            # To jest wiersz z kwotą. Teraz musimy znaleźć:
            # A. Nazwisko (powinno być w kolumnie 0 lub 1 dzięki ffill)
            kierowca = None
            
            # Sprawdzamy kolumnę 1 (B) - tam na screenie są nazwiska
            kandydat_1 = str(row.iloc[1]).strip()
            # Sprawdzamy kolumnę 0 (A)
            kandydat_0 = str(row.iloc[0]).strip()
            
            # Wybieramy to co wygląda na nazwisko (nie jest puste i nie jest słowem kluczowym)
            def czy_nazwisko(txt):
                t = txt.upper()
                return len(t) > 2 and not any(s in t for s in SŁOWA_KLUCZOWE) and "STAWKA" not in t
                
            if czy_nazwisko(kandydat_1):
                kierowca = kandydat_1
            elif czy_nazwisko(kandydat_0):
                kierowca = kandydat_0
            
            if kierowca:
                # B. Szukamy kwoty w tym wierszu
                wartosc = 0.0
                found_val = False
                
                # Iterujemy po kolumnach od 2 do końca
                for col_idx in range(2, len(row)):
                    val_str = str(row.iloc[col_idx]).replace(',', '.').replace(' ', '').replace('zł', '').replace('pln', '')
                    
                    # Ignorujemy puste, napisy i zera
                    if not val_str or any(c.isalpha() for c in val_str):
                        continue
                        
                    try:
                        v = float(val_str)
                        if v > 0: # Bierzemy pierwszą dodatnią liczbę w wierszu KWOTA
                            wartosc = v
                            found_val = True
                            break
                    except:
...
def pobierz_przypisania_webfleet(account, username, password, data_start, data_stop):
    # 1. Sprawdzenie danych logowania
...
def to_excel_extended(df_summary, df_subiekt_raw, df_fuel_raw):
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        # --- ARKUSZE SZCZEGÓŁOWE (BEZ ZMIAN LOGICZNYCH, TYLKO FORMAT) ---
        pojazdy_subiekt = set()
        ...
        for pojazd in wszystkie_pojazdy:
            ...
            if df_subiekt_raw is not None and not df_subiekt_raw.empty:
                ...
                if not sub_data.empty:
                    ...
                    # Odwracamy znaki kosztów dla czytelności (koszt jako ujemny w cashflow)
...
def render_admin_content(conn, wybrana_firma):
    ...
    with st.expander("📡 Konfiguracja Webfleet API", expanded=False):
        ...
        with st.form("webfleet_config"):
            inp_account = st.text_input("Account", value=acc if acc else "")
            inp_username = st.text_input("Username", value=user if user else "")
            inp_password = st.text_input("Password", value=pw if pw else "", type="password")
            if st.form_submit_button("Zapisz"):
                zapisz_ustawienia_api(conn, inp_account, inp_username, inp_password)
    
    st.divider()

    # 3. ANALIZA WYNAGRODZEŃ (MULTI-FILE + NAZWY MIESIĘCY Z PLIKU)
    st.markdown("### 💰 Analiza Wynagrodzeń")
    st.caption("Wgraj jeden lub wiele plików. System spróbuje rozpoznać miesiąc z nazwy pliku (np. 'Grudzień 2025.xlsx') lub nazwy arkusza.")
    ...
    with st.container(border=True):
        ...
        with col_w1:
            rok_analizy = st.number_input("Rok domyślny", min_value=2023, max_value=2030, value=2025) 
            # ZMIANA: accept_multiple_files=True
            pliki_plac = st.file_uploader("Wgraj pliki Excel", type=['xlsx', 'xls'], accept_multiple_files=True)
            ...
            for plik in pliki_plac:
                try:
                    ...
                    for arkusz in xls_file.sheet_names:
                        # Przekazujemy nazwę pliku do funkcji daty!
                        start_auto, stop_auto = wyznacz_zakres_dat_z_arkusza(arkusz, plik.name, rok_analizy)
                        
                        if start_auto:
                            etykieta = f"📅 {start_auto.strftime('%B %Y')} | Plik: {plik.name} | Arkusz: {arkusz}"
                        else:
                            etykieta = f"⚠️ BRAK DATY | Plik: {plik.name} | Arkusz: {arkusz}"
                        ...
                        lista_etykiet.append(etykieta)
                except Exception as e:
                    st.error(f"Błąd pliku {plik.name}: {e}")

            if lista_etykiet:
                wybrane_opcje = st.multiselect("Wybierz okresy do przeliczenia:", options=lista_etykiet, default=[l for l in lista_etykiet if "⚠️" not in l])

                if st.button("🚀 Oblicz Wybrane", type="primary"):
                    if not wybrane_opcje:
                        st.warning("Wybierz co najmniej jeden okres.")
                    else:
                        st.session_state['temp_wynagrodzenia_all'] = None 
                        acc, user, pw = pobierz_ustawienia_api(conn)
                        kurs_eur = pobierz_kurs_eur_pln()
                        st.session_state['current_eur_rate'] = kurs_eur or 4.30
                        ...
                        if not acc:
                            ...
                            for i, etykieta in enumerate(wybrane_opcje):
                                dane = mapa_opcji[etykieta]
                                if not dane['start']: continue
                                
                                pask.progress((i / len(wybrane_opcje)), text=f"Przetwarzam: {etykieta}")
                                
                                # 1. Pobierz Webfleet
                                df_wf = pobierz_przypisania_webfleet(acc, user, pw, dane['start'], dane['stop'])
                                
                                # 2. Parsuj Excel (Nowa logika ze scalonymi komórkami)
                                df_sheet = pd.read_excel(dane['excel_obj'], sheet_name=dane['sheet'], header=None)
                                df_place = parsuj_dataframe_plac(df_sheet)
                                
                                if df_place.empty:
                                    st.warning(f"Brak danych płacowych w: {etykieta}")
                                    continue
                               ...
                               # --- POCZĄTEK POPRAWKI (Dopasowanie Imie Nazwisko <-> Nazwisko Imie) ---
                                ...
                                if not merged.empty:
                                    ...
                                    # Fix dla braku GPS
                                    mask_brak = merged['pojazd'] == "BRAK (Wybierz)"
                                    merged.loc[mask_brak, 'koszt_przypisany'] = merged.loc[mask_brak, 'kwota_total']
        ...
        if st.session_state.get('temp_wynagrodzenia_all') is not None:
            st.divider()
            df_ed = st.session_state['temp_wynagrodzenia_all']
            opcje = st.session_state.get('wszystkie_znane_pojazdy', DODATKOWE_POJAZDY)
            
            edited = st.data_editor(
                df_ed,
                column_config={
                    "pojazd": st.column_config.SelectboxColumn("Pojazd", options=opcje, required=True),
                    "Pełna Wypłata": st.column_config.NumberColumn(format="%.2f zł", disabled=True),
                    "koszt_przypisany": st.column_config.NumberColumn("Koszt PLN", format="%.2f zł"),
                    "koszt_eur_est": st.column_config.NumberColumn("Koszt EUR (Est)", format="%.2f €", disabled=True),
                    "Kierowca": st.column_config.TextColumn(disabled=True),
            ...
            if st.button("💾 ZAPISZ WSZYSTKO DO BAZY", type="primary"):
                try:
                    db_df = edited.copy()
                    # Mapowanie kolumn do bazy
                    ...
                    # Logika firmy
                    def get_company(r):
                        p = str(r['pojazd']).upper().replace("-","").replace(" ","")
                        if p in UNIX_FLOTA_CONFIG or 'TRUCK' in p or 'KACPER' in p: return 'UNIX-TRANS'
                        return 'HOLIER'
                    db_df['firma'] = db_df.apply(get_company, axis=1)
                    
                    cols = ['data_transakcji', 'identyfikator', 'kwota_netto', 'kwota_brutto', 'waluta', 'ilosc', 'produkt', 'typ', 'zrodlo', 'kraj', 'firma', 'kontrahent']
                    db_df[cols].to_sql(NAZWA_TABELI, conn.engine, if_exists='append', index=False, schema=NAZWA_SCHEMATU)
                    st.success("Zapisano!")
                    time.sleep(1)
                    st.rerun()
                except Exception as e:
...
def render_raport_content(conn, wybrana_firma):
    ...
    try:
        ...
        if dane_z_bazy_full.empty:
            ...
            # --- ZAKŁADKA WYNAGRODZENIA (Z BAZY) ---
            with sub_tab_wynagrodzenia:
                # Pobieramy dane o typie WYNAGRODZENIE z df_z_bazy_full (przed filtracją prepared)
                df_w = dane_z_bazy_full[dane_z_bazy_full['typ'] == 'WYNAGRODZENIE'].copy()
                
                if df_w.empty:
                    st.info("Brak danych o wynagrodzeniach w tym okresie. Przejdź do Panelu Administratora, wgraj plik Excel i zapisz wyniki do bazy.")
                else:
                    # Konwersja na liczby dla pewności
                    df_w['kwota_brutto'] = pd.to_numeric(df_w['kwota_brutto'], errors='coerce').fillna(0)
                    
                    suma_wyn = df_w['kwota_brutto'].sum()
                    st.metric("Łączne Wynagrodzenia (Przypisane)", f"{suma_wyn:,.2f} PLN", border=True)
                    
                    # Agregacja per pojazd
                    df_w['pojazd_clean'] = bezpieczne_czyszczenie_klucza(df_w['identyfikator'])
                    grp_w = df_w.groupby('pojazd_clean')['kwota_brutto'].sum().sort_values(ascending=False).reset_index()
                    grp_w.columns = ['Pojazd', 'Kwota (PLN)']
                    grp_w.insert(0, 'Lp.', range(1, 1 + len(grp_w)))
                    
                    st.dataframe(grp_w.style.format({'Kwota (PLN)': '{:,.2f} PLN'}), use_container_width=True, hide_index=True)
                    
                    with st.expander("Szczegóły zapisów w bazie"):
                        st.dataframe(df_w[['data_transakcji', 'identyfikator', 'kwota_brutto', 'zrodlo']])
...
def render_rentownosc_content(conn, wybrana_firma):
        ...
        if not plik_analizy:
            ...
            with st.spinner("Obliczam VAT i Rentowność..."):
                ...
                final = pd.concat([
                    ...
                    df_koszty_wynagr,
                    df_koszty_subiekt_brutto, df_koszty_subiekt_netto,
                    df_klienci
                ], axis=1).fillna(0)
    ...
    if st.session_state.get('raport_gotowy'):
        ...
        m4.metric("BILANS VAT (Zwrot/Zapłata)", f"{bilans_vat:,.2f} EUR", 
                  delta="Na plus (zwrot)" if bilans_vat > 0 else "Na minus (do zapłaty)", 
                  border=True)

        # 3. Tabela Główna
        st.markdown("### Tabela Rentowności z VAT")
        
        # Konfiguracja kolumn do wyświetlenia
        cols_order = [
            'Kontrahent', 
            'PRZYCHOD_EUR', 'VAT_PRZYCHOD_EUR',
            'KOSZT_PALIWO_EUR', 'VAT_PALIWO_EUR', 
            'KOSZT_INNE_EUR', 'VAT_INNE_EUR',
            'KOSZT_KIEROWCA_EUR', 
            'VAT_BILANS_EUR', 'ZYSK_EUR'
        ]
        ...
        show_df.insert(0, 'Lp.', range(1, 1+len(show_df)))
        ...
        st.dataframe(
            ...
            column_config={
                ...
                "PRZYCHOD_EUR": st.column_config.NumberColumn("Przychód (Brutto)", format="%.2f €"),
                "VAT_PRZYCHOD_EUR": st.column_config.NumberColumn("VAT Sprz. (-)", format="%.2f €"),
                
                "KOSZT_PALIWO_EUR": st.column_config.NumberColumn("Paliwo (Brutto)", format="%.2f €"),
                "VAT_PALIWO_EUR": st.column_config.NumberColumn("VAT Paliwo (+)", format="%.2f €"),
                
                "KOSZT_INNE_EUR": st.column_config.NumberColumn("Inne (Brutto)", format="%.2f €"),
                "VAT_INNE_EUR": st.column_config.NumberColumn("VAT Inne (+)", format="%.2f €"),

                "KOSZT_KIEROWCA_EUR": st.column_config.NumberColumn("Kierowca", format="%.2f €"),
                
                "VAT_BILANS_EUR": st.column_config.NumberColumn("BILANS VAT", format="%.2f €"),
                "ZYSK_EUR": st.column_config.NumberColumn("ZYSK (Cash)", format="%.2f €"),
            },
            use_container_width=True,
            hide_index=True,
            height=500
        ...
        st.download_button(
            label="📥 Pobierz PEŁNY Raport Excel (Ze szczegółami VAT)",
            data=excel_bytes,
            file_name=f"rentownosc_VAT_{wybrana_firma}_{date.today()}.xlsx",
        ...
        wybrany = st.selectbox("Wybierz pojazd do analizy:", lista_aut)
        ...
        if wybrany != "--- Wybierz pojazd ---":
            ...
            d2.metric("Bilans VAT", f"{row['VAT_BILANS_EUR']:,.2f} €", help="VAT Paliwo + VAT Inne - VAT Sprzedaż")
            d3.metric("VAT ze Sprzedaży", f"{row['VAT_PRZYCHOD_EUR']:,.2f} €")
            d4.metric("VAT z Zakupów", f"{(row['VAT_PALIWO_EUR'] + row['VAT_INNE_EUR']):,.2f} €")
            d5.metric("Kierowca", f"{row['KOSZT_KIEROWCA_EUR']:,.2f} €")

            details = []
            
            # Detale z Subiekta (Pokazujemy Brutto i Netto)
            if df_raw_analiza is not None:
                sub_rows = df_raw_analiza[df_raw_analiza['pojazd_clean'] == wybrany].copy()
                if not sub_rows.empty:
                    # Odwracamy koszty dla wizualizacji
                    mask_koszt = sub_rows['typ'] == 'Koszt (Subiekt)'
                    sub_rows.loc[mask_koszt, 'kwota_brutto_eur'] *= -1
                    sub_rows.loc[mask_koszt, 'kwota_netto_eur'] *= -1
                    
                    sub_rows = sub_rows[['data', 'opis', 'typ', 'kwota_brutto_eur', 'kwota_netto_eur']]
                    sub_rows.columns = ['Data', 'Opis', 'Typ', 'Brutto (EUR)', 'Netto (EUR)']
                    details.append(sub_rows)

            # Detale z Bazy (Paliwo/Wynagr)
            ...
            if df_baza_raw is not None:
                ...
                if not baza_rows.empty:
                    ...
                    baza_rows['kwota_netto_eur'] = -baza_rows['kwota_netto_eur'].abs()
                    ...
                    baza_view['Brutto (EUR)'] = baza_rows['kwota_brutto_eur']
                    baza_view['Netto (EUR)'] = baza_rows['kwota_netto_eur']
                    details.append(baza_view)

            if details:
                full_details = pd.concat(details).sort_values(by='Data', ascending=False).reset_index(drop=True)
                full_details['VAT (Calc)'] = full_details['Brutto (EUR)'] - full_details['Netto (EUR)']
                
                st.dataframe(
                    full_details.style.format({'Brutto (EUR)': '{:,.2f} €', 'Netto (EUR)': '{:,.2f} €', 'VAT (Calc)': '{:,.2f} €'})
                                      .map(lambda x: 'color: green' if x > 0 else 'color: red', subset=['Brutto (EUR)']),
                    use_container_width=True,
                    hide_index=True,
                    column_config={"Data": st.column_config.DateColumn("Data")}
                )
            else:
                st.info("Brak szczegółowych transakcji dla tego pojazdu.")
